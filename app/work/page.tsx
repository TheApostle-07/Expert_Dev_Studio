        ...
        <div className="relative z-10 mx-auto w-full max-w-none px-6 sm:px-8 lg:px-12">
          <MotionSection amount={0.3} className="pt-24 pb-16 sm:pt-28 sm:pb-20 md:pt-32 md:pb-24 text-center">
            ...
            <motion.h1
              id="work-hero-title"
              variants={fadeInUp}
              className="mx-auto mt-3 sm:mt-4 max-w-5xl font-display text-4xl font-semibold leading-[1.08] tracking-[-0.02em] sm:text-5xl md:text-6xl xl:text-7xl [text-wrap:balance]"
              style={{ overflow: "visible" }}
            >
              <>
                Luxury in look.
                <span className="relative z-10 block mt-2 bg-transparent text-cerulean">Relentless on performance.</span>
              </>
            </motion.h1>
                  ...
                  <button
                    ...
                    className={[
                      ...
                      active
                        ...
                        : "border-black/10 bg-white/80 backdrop-blur-sm text-black/70 hover:bg-white/70",
                    ].join(" ")}
                  >
                    {f}
                  </button>
      ...
      <section className="container mx-auto px-4 py-12 sm:px-6 lg:px-8" aria-labelledby="work-grid-title">
        <MotionSection amount={0.25}>
          ...
          <div className="mt-8 grid auto-rows-fr gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filtered.length === 0 ? (
              <div className="col-span-full rounded-2xl border border-black/10 bg-white/80 backdrop-blur-sm p-8 text-center text-sm text-black/60">
...
function ProjectCard({ p }: { p: Project }) {
  return (
    <motion.article
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-black/10 bg-white/80 backdrop-blur-sm shadow-[0_4px_20px_-8px_rgba(0,0,0,0.08)]"
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="inline-flex shrink-0 items-center rounded-full border border-black/10 bg-white/80 backdrop-blur-sm px-2 py-1 text-[11px] font-medium text-prussian">
            {p.tag}
          </span>
        </div>
        ...
        {p.metrics && p.metrics.length > 0 && (
          <ul className="mt-3 flex flex-wrap gap-2 text-[11px]">
            {p.metrics.map(([k, v]) => (
              <li
                key={k + v}
                className="inline-flex items-center gap-1 rounded-full border border-black/10 bg-white/80 backdrop-blur-sm px-2 py-1 text-black/70"
              >
        ...
        <div className="mt-auto pt-4 flex items-center gap-2">
          <a
            ...
            rel="noreferrer"
            className={[
              "inline-flex items-center gap-1.5 rounded-lg border border-black/10 bg-white/80 backdrop-blur-sm px-3 py-2 text-xs font-medium",
